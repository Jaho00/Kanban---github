				...
				<view class="col-2 wrap-line no-gutters">
					...
					<view class="col-12 no-gutters rate-content">
						...
						<view class="allsize justify-content-center row no-gutters" id="zhxl">
							<dv-decoration-9 class="dvContent">{{ item.OEE }}%</dv-decoration-9>
						</view>
					</view>
					<view class="col-12 no-gutters rate-content">
						<view class="col-12 row no-gutters justify-content-center">
						...
						<view class="allsize justify-content-center row no-gutters" id="lpl">
							<dv-decoration-9 class="dvContent">{{ item.YIELD_RATE }}%</dv-decoration-9>
						</view>
					</view>
					<view class="col-12 no-gutters rate-content">
						<view class="col-12 row no-gutters justify-content-center">
						...
						<view class="allsize justify-content-center row no-gutters" id="xnjd">
							<dv-decoration-9 class="dvContent">{{ item.PO_RATE }}%</dv-decoration-9>
						</view>
					</view>
					<view class="col-12 no-gutters rate-content">
						<view class="col-12 row no-gutters justify-content-center">
						...
						<view class="allsize justify-content-center row no-gutters" id="sjjd">
							<dv-decoration-9 class="dvContent">{{ item.TO_RATE }}%</dv-decoration-9>
						</view>
					</view>
				</view>
				<!-- 中间区域 -->
						...
						<view class="row no-gutters devinfo">
							<view class="col-6 right-border">
								<view style="width: 100%; height: 100%;">
									 <video style="width: 100%; height: 100%;" :src="devInfo.videoURL"
						...
						<view class="row no-gutters list_content">
							<view class="row col-12 no-gutters produce_title">
								...
								<view class="col-3 row no-gutters">
									...
									<text class="align-self-center" style="margin-left: 5%;">生产任务：{{ productList.length }}</text>
								</view>
							</view>
							<view class="row col-12 no-gutters produce_content">
								<view class="col-12 produce_wrapper">
									<view class="row no-gutters col-12" style="padding: 2% 2%;" v-for="ele in productList" :key="ele.FJOB_NO">
					...
					<view class="col-12">
							...
							<view class="col-12 row no-gutters right_top_height">
								...
								<view class="col-12 row no-gutters right_top_height">
									<view class="progress col-12" style="height: 15px;font-size: .15rem;">
										...
										v-for="ele in gldata" :key="ele.Order" aria-valuemin="0" aria-valuemax="100"></view>
									</view>
						...
						<view class="row no-gutters right_down">
							...
							<view class="row col-12 no-gutters produce_content">
								...
								<view style="height: 100%; width: 100%;" id="zqt">
...
<script>
	...
	export default {
		...
		methods: {
			...
			drawCircle(){
				this.item.OEE = (+this.item.OEE).toFixed(2);
				this.item.YIELD_RATE = (+this.item.YIELD_RATE).toFixed(2);
				this.item.PO_RATE = (+this.item.PO_RATE).toFixed(2);
				this.item.TO_RATE = (+this.item.TO_RATE).toFixed(2);
				this.getzqtData();
				// this.LPLconfig = {value: this.item.YIELD_RATE,borderRadius: 10,localGradient: true };
			...
			getzqtData(){
				uni.request({
					...
					success: (res) => {
						if (res.data.IsSuccess) {
							this.drawZQT(res.data.Data);
						}
					},
				});
			},
			drawZQT(target){
				const dom = document.querySelector('#zqt');
				if (!zqtChartObj) {
					zqtChartObj = this.$echarts.init(dom);
				}
				let series = [];
				let legend = [];
				//处理正式数据
				...
				uni.request({
					...
					success: (res) => {
						if (res.data.IsSuccess) {
							...
							if(this.devInfo.FDEVPHOTO){
								let tempObj = JSON.parse(this.devInfo.FDEVPHOTO);
								if(tempObj.length > 0){
									this.devInfo.videoURL = this.basePath + tempObj[0].path;
								}else{
									this.devInfo.videoURL = ""
								}
							}else{
								this.devInfo.videoURL = ""
							}
