		...
		<dv-full-screen-container>
			...
			<view class="row addpadding tableContent no-gutters" style="overflow: hidden;margin-top: .5%;">
				...
				<view class="col-9 row no-gutters tdContent align-content-start">
					<view class="addpadding setHeight col-3" v-for="item in showdataList" :key="item.id">
						<view class="row no-gutters" :class="item.SY_STATUS  == 3 ? 'shapeGray' : item.SY_STATUS == 2 ? 'shapeGreen' : 'shapeRed'"
						 @click="openPage(item)">
							...
							<view class="row Content-text no-gutters">
								...
								<!-- <u-line-progress :percent="item.FCOMP_RATE" :show-percent="true" style="height: .2rem;font-size: 14px; font-weight: normal; color: #000;" fontSize="0.2" :active-color="item.FCOMP_RATE > 100 ? '#FA3534':'#19be6b'">
								{{item.FDEVCODE}}
								</u-line-progress> -->
							</view>
		...
		methods: {
			...
			getTableList(){
				...
				uni.request({
					...
					data: {
						zhid: uni.getStorageSync('UserInfo').ZHID,
						kbcode: 'sy-nozs',//this.titleObj.FCODE, //车间代码
					},
					method: 'POST',
					header: {
						'content-type': 'application/x-www-form-urlencoded' //自定义请求头信息
			...
			setTableName(){
				...
				console.log(this.labelArr,this.NameList)
			},
			updateOEE(){
				uni.showLoading({
					title: '更新中'
