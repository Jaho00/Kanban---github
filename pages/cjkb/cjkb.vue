	...
	<view class="container-fluid setfull">
		<dv-full-screen-container>
			...
			<view class="row addpadding" style="overflow: hidden;height: 33%;">
				<view class="col-9 row no-gutters" style="height: 100%;">
					...
					<view class="col-12" style="width: 100%;overflow: hidden;height: 84%;">
						<view class="" :animation="GDanimate">
							<u-table class="scrollList" style="width: 100%;overflow: hidden;" bg-color="transparent" color="#FFFFFF"
							 border-color="#052D84">
								<u-tr v-for="(item,index) in DayPZJKoption" :key="index" class="GdTrHeight" :style="{backgroundColor: showBG === index ? 'blue':'inherit'}">
									<u-td width="12%" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size: .17rem;">
										{{ item.FCO }}
									</u-td>
									<u-td width="12%" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size: .2rem;">
										{{ item.FPRD_NAME }}
									</u-td>
									<u-th width="12%" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size: .2rem;">
										{{ item.FPRD_NO }}
									</u-th>
									<u-th width="12%" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size: .2rem;">
										{{ item.FDEVICE_NAME }}
				...
				<view class="col row no-gutters ml-1 charttitle" style="font-size: .3rem;text-align: center;height: 100%;">
			...
			<view class="row addpadding mt-1 tableContent tableHeight" style="overflow: hidden;">
				...
				<view class="col-9" style="width: 100%;overflow: hidden;height: 90%;border: 1px solid #052D84;">
					<view class="" :animation="animationData">
						<u-table class="scrollList GXtableHeight" style="width: 100%;overflow: hidden;" bg-color="transparent" color="#FFFFFF"
						 border-color="#052D84">
							<u-tr v-for="(item,index) in GXList" :key="item.FID" class="GxTrHeight">
								<u-td width="30%" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" class="charttitle">
									{{ item.FWORKNAME }}
								</u-td>
								<u-td class="charttitle">
									{{ item.TASK_QTY }}
				...
				<view class="row" style="height: 89%;width: 23%;">
					<view class="col-12" style="height: 90%;margin-left: 7%;">
						<canvas canvas-id="canvasPie" id="canvasPie" style="width: 100%;height: 100%;" class="charts blyyDiv"></canvas>
					</view>
				</view>
			</view>
			...
			<view class="row addpadding" style="height: 29%;">
				...
				<view class="col row no-gutters charttitle" style="font-size: .3rem;text-align: center;height: 100%;">
					...
					<view class="col-12" style="height: 90%;">
						...
						<canvas canvas-id="BFPie" id="BFPie" style="width: 100%;height: 100%;" class="charts bfyyDiv"></canvas>
					</view>
				</view>
			</view>
		</dv-full-screen-container>
...
<script>
	...
	export default {
		...
		methods: {
			...
			scrollGD() {
				setTimeout(() => {
					if (_self.DayPZJKoption.length > 8) {
						this.count++;
					...
					success: (res) => {
						if (res.data.IsSuccess) {
							...
							res.data.Data.forEach((ele, index) => {
								...
								ele.YieldRate = (ele.QUA_QTY + ele.BAD_QTY) == 0 ? 0 : (ele.QUA_QTY / (ele.QUA_QTY + ele.BAD_QTY) * 100).toFixed(
									2);
							});
